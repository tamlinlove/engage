...
class GazeConfidenceDM(DecisionMaker):
    ...
    def decide(self,state:EngageState,discretise=True):
        ...
        if state.waiting:
            ...
            # Find the bodies that maximise the mutual gaze
            '''
    ...
    def calculate_scores(self,state:EngageState,new_bodies):
        ...
        for body in new_bodies:
            ...
            if mg is None:
                scores[body] = -9999999
                continue
            dmg = self.discrete_score(mg)/self.max_pec
            pec = state.pose_confidences[body]
            if pec is None:
                scores[body] = 0
                continue
            dpec = self.discrete_score(pec)/self.max_pec
            scores[body] = dpec*dmg
        return scores
